    ...
    <div className='max-w-4xl mx-auto'>
        <div className='text-[14px] prose prose-slate dark:prose-invert max-w-none'>
            <ReactMarkdown 
                ...
                blockquote({ children}) {
                    return <blockquote className='border border-l-4 border-gray-200 pl-4 italic my-4'>{children}</blockquote>
                },
                h1({ children}) {
                ...
                hr() {
                    return <hr className='my-6 border border-gray-200'/>
                },
                img({ src, alt}) {
    ...
    return <div className='relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200'>
                <div className='flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200'>
                    <div className='flex items-center space-x-2'>
                        ...
                        <span className='text-xs font-medium text-gray-600 uppercase tracking-wide'>{language || 'code'}</span>
                    </div>
                    <button className='text-gray-500 hover:text-gray-700 focus:outline-none relative group' onClick={copyCode} aria-label='Copy code'>
                        ...
                        {copied && ( <span className='absolute -top-8 right-8 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition'>Copied!</span> )}
                    </button>
                </div>
                <SyntaxHilighter
                    language={language}
